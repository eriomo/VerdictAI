...
// ── Auth middleware — verifies JWT from client ────────────────────────────────
...
app.post('/api/ai', requireAuth, async (req, res) => {
  ...
  // Check usage limit for free tier users
  try {
    ...
    // Don't block on profile errors — still serve the request
      ...
      if (ct.includes('json')) {
        ...
        (data.results || data.items || []).slice(0, 8).forEach(item => {
          results.push({
            ...
            year: item.year || '',
            url: item.url || 'https://africanlii.org',
